...
Another common error you'll possibly see will occur with step 5 (Align LC-MS 
Features with the Mass Tags):
  ActiveX component (Error starting alignment: ActiveX component can't create object)

To fix this, you need to manually register the MassMatchCOM.dll by following these steps:
* Exit Viper
* Start the Windows Command prompt as an administrator
  * In Windows XP, find the shortcut to the Command Prompt in the start menu, right click, and choose PM-Run Elevated
  * In Windows 7, find the shortcut to the Command Prompt in the start menu, right click, and choose Run As Administrator
* Change the directory to C:\program files\Viper using either of these commands (depends on where VIPER is installed):
  * cd "\program files\viper"
  * cd "\program files (x86)\viper"
...
.PEK files, generated by ICR-2LS (download at http://omics.pnl.gov/software/)
...
.CSV files, generated by Decon2LS (available at http://omics.pnl.gov/software/)
