...
' Api Functions
' Note that GetTickCount returns a negative number after 24 days of computer Uptime and resets to 0 after 48 days
Private Declare Function GetTickCountOriginal Lib "kernel32" Alias "GetTickCount" () As Long
...
Public Function CLngRoundUp(dblValue As Double) As Long
    ' Equivalent to the VB.NET function Math.Ceiling()
...
Public Function CStrSafe(strWork As String) As String
    On Error GoTo CStrSafeErrorHandler
...
Public Function ConstructFormatString(ByVal dblThisValue As Double, Optional ByRef intDigitsInFormattedValue As Integer) As String
    ...
    strWork = DoubleToStringScientific(dblThisValue, 3)
...
Public Function GetTickCount() As Long
    ' Since GetTickCount returns a negative number after 24 days of computer Uptime, we
    '  we'll turn that into an increasing positive number by adding to 2147483647
    ' Thus, GetTickCount will reset to 0 every 24 days (since GetTickCountOriginal changes
    '   to a positive number after 48 days)
    
    Dim lngTickCount As Long
    
    lngTickCount = GetTickCountOriginal()
    If lngTickCount < 0 Then lngTickCount = 2147483647 + lngTickCount
    
    GetTickCount = lngTickCount
End Function
...
Public Function FormatNumberAsString(ByVal dblNumber As Double, Optional lngMaxLength As Long = 10, Optional lngMaxDigitsOfPrecision As Long = 8, Optional blnUseScientificWhenTooLong As Boolean = True) As String
    ...
    dblNumber = CDbl(Format(dblNumber, strZeroes & "E+00"))
    ...
    If Len(strNumberAsText) > lngMaxLength Then
        If blnUseScientificWhenTooLong Then
            ...
            strNumberAsText = Format(dblNumber, "0." & strZeroes & "E+00")
        Else
            If lngMaxLength < 3 Then lngMaxLength = 3
...
On Error GoTo FileExistsErrorHandler
...
Public Sub ShellSortDouble(ByRef dblArray() As Double, ByVal lngLowIndex As Long, ByVal lngHighIndex As Long)
    Dim lngCount As Long
...
Public Function ShellSortDoubleWithParallelLong(ByRef dblArray() As Double, ByRef lngParallelData() As Long, ByVal lngLowIndex As Long, ByVal lngHighIndex As Long) As Boolean
...
On Error GoTo ShellSortDoubleWithParallelLongErrorHandler

' sort array[lngLowIndex..lngHighIndex]
' Also sort lngParallelData() in parallel
    ...
    Do While lngIncrement > 0
        ...
        For lngIndex = lngLowIndex + lngIncrement To lngHighIndex
            dblCompareVal = dblArray(lngIndex)
            lngCompareVal = lngParallelData(lngIndex)
            For lngIndexCompare = lngIndex - lngIncrement To lngLowIndex Step -lngIncrement
                ' Use <= to sort ascending; Use > to sort descending
                If dblArray(lngIndexCompare) <= dblCompareVal Then Exit For
                dblArray(lngIndexCompare + lngIncrement) = dblArray(lngIndexCompare)
                lngParallelData(lngIndexCompare + lngIncrement) = lngParallelData(lngIndexCompare)
            Next lngIndexCompare
            dblArray(lngIndexCompare + lngIncrement) = dblCompareVal
            lngParallelData(lngIndexCompare + lngIncrement) = lngCompareVal
        Next lngIndex
        lngIncrement = lngIncrement \ 3
    Loop

    ShellSortDoubleWithParallelLong = True
    Exit Function

ShellSortDoubleWithParallelLongErrorHandler:
    Debug.Assert False
    ShellSortDoubleWithParallelLong = False
End Function

Public Function ShellSortLongWithParallelDouble(ByRef lngArray() As Long, ByRef dblParallelData() As Double, ByVal lngLowIndex As Long, ByVal lngHighIndex As Long) As Boolean
    ' Return True if success, false if error
...
On Error GoTo ShellSortLongWithParallelDoubleErrorHandler

' sort array[lngLowIndex..lngHighIndex]
' Also sort dblParallelData() in parallel
    ...
    Do While lngIncrement > 0
        ...
        For lngIndex = lngLowIndex + lngIncrement To lngHighIndex
            lngCompareVal = lngArray(lngIndex)
            dblCompareVal = dblParallelData(lngIndex)
            For lngIndexCompare = lngIndex - lngIncrement To lngLowIndex Step -lngIncrement
                ' Use <= to sort ascending; Use > to sort descending
                If lngArray(lngIndexCompare) <= lngCompareVal Then Exit For
                lngArray(lngIndexCompare + lngIncrement) = lngArray(lngIndexCompare)
                dblParallelData(lngIndexCompare + lngIncrement) = dblParallelData(lngIndexCompare)
            Next lngIndexCompare
            lngArray(lngIndexCompare + lngIncrement) = lngCompareVal
            dblParallelData(lngIndexCompare + lngIncrement) = dblCompareVal
        Next lngIndex
        lngIncrement = lngIncrement \ 3
    Loop

    ShellSortLongWithParallelDouble = True
    Exit Function

ShellSortLongWithParallelDoubleErrorHandler:
    Debug.Assert False
    ShellSortLongWithParallelDouble = False
End Function


Public Sub ShellSortLongWithParallelSingle(ByRef lngArray() As Long, ByRef sngParallelArray() As Single, ByVal lngLowIndex As Long, ByVal lngHighIndex As Long)
    Dim lngCount As Long
    Dim lngIncrement As Long
    Dim lngIndex As Long
    Dim lngIndexCompare As Long
...
' sort array[lngLowIndex..lngHighIndex]
    ...
    Do While lngIncrement > 0
        ...
        For lngIndex = lngLowIndex + lngIncrement To lngHighIndex
            lngCompareVal = lngArray(lngIndex)
            sngSwap = sngParallelArray(lngIndex)
            For lngIndexCompare = lngIndex - lngIncrement To lngLowIndex Step -lngIncrement
                ' Use <= to sort ascending; Use > to sort descending
                If lngArray(lngIndexCompare) <= lngCompareVal Then Exit For
                lngArray(lngIndexCompare + lngIncrement) = lngArray(lngIndexCompare)
                sngParallelArray(lngIndexCompare + lngIncrement) = sngParallelArray(lngIndexCompare)
            Next lngIndexCompare
            lngArray(lngIndexCompare + lngIncrement) = lngCompareVal
            sngParallelArray(lngIndexCompare + lngIncrement) = sngSwap
        Next lngIndex
        lngIncrement = lngIncrement \ 3
    Loop
...
Public Sub QSDoubleTest()
    ' This sub uses VB class QSDouble.cls
    
    Dim objSort As QSDouble
    
    Dim dblData() As Double
    Dim lngPointers() As Long
    Dim lngIndex As Long
    
    ReDim dblData(10)
    ReDim lngPointers(10)
    
    Debug.Print "Unsorted"
    For lngIndex = 0 To 10
        dblData(lngIndex) = Rnd(1) * 50
        lngPointers(lngIndex) = lngIndex
        Debug.Print dblData(lngIndex), lngPointers(lngIndex)
    Next lngIndex
    
    Set objSort = New QSDouble
    If objSort.QSAsc(dblData, lngPointers) Then
...
Public Sub SizeAndCenterWindow(frmThisForm As VB.Form, Optional intCenterMode As Integer = 0, Optional lngWindowWidth As Long = -1, Optional lngWindowHeight As Long = -1, Optional boolSizeAndCenterOnlyOncePerProgramSession As Boolean = True, Optional intDualMonitorToUse As Integer = -1)
    ...
    With frmThisForm
        ...
        Case cWindowBottomCenter
            lngWindowLeftToSet = (lngWorkingAreaWidth - .width) \ 2
            lngWindowTopToSet = lngWorkingAreaHeight - .Height - 1024
