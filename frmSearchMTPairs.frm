...
Begin VB.Form frmSearchMTPairs 
   ...
   Begin VB.Image Image1 
      Height          =   480
      Left            =   2940
      Picture         =   "frmSearchMTPairs.frx":0000
...
Dim mMatchStatsCount As Long                                'count of Pair-ID matches
Dim mUMCMatchStats() As udtUMCMassTagMatchStats             ' 0-based array

' The following hold match stats for each individual Pair
Dim mCurrIDCnt As Long
Dim mCurrIDMatches() As udtUMCMassTagRawMatches         ' 0-based array
...
Dim PIDInd1() As Long           'first index in ID arrays for pair (pointer into mUMCMatchStats)
Dim PIDInd2() As Long           'last index in ID arrays for pair (pointer into mUMCMatchStats)
...
If Len(ExpUmcSPName) = 0 Then ExpUmcSPName = "AddFTICRUmc"
Debug.Assert ExpUmcSPName = "AddFTICRUmc"

If Len(ExpUmcMatchSPName) = 0 Then ExpUmcMatchSPName = "AddFTICRUmcMatch"
Debug.Assert ExpUmcMatchSPName = "AddFTICRUmcMatch"

If Len(ExpQuantitationDescription) = 0 Then ExpQuantitationDescription = "AddQuantitationDescription"
Debug.Assert ExpQuantitationDescription = "AddQuantitationDescription"

If Len(ExpAnalysisSPName) = 0 Then ExpAnalysisSPName = "AddMatchMaking"
Debug.Assert ExpAnalysisSPName = "AddMatchMaking"

' September 2004: Unused Variable
''If Len(ExpPeakSPName) = 0 Then ExpPeakSPName = "AddFTICRPeak"
...
''so that it can be communicated to other functions
'Dim Res As Long
'Dim i As Long
'On Error Resume Next
...
'       End If
'       ReDim .PInd(mMatchStatsCount - 1)
'       ReDim .PIDInd(mMatchStatsCount - 1)
'       .Cnt = mMatchStatsCount
'       For i = 0 To mMatchStatsCount - 1
'           .PInd(i) = mUMCMatchStats(i).PairIndex
'           .PIDInd(i) = mUMCMatchStats(i).IDIndex
'       Next i
...
strLineOut = strLineOut & "SLiC Score" & strSepChar & "Del_SLiC"
        ...
        If blnShowExcludedPairs Or (GelP_D_L(CallerID).Pairs(lngPairInd).STATE <> glPAIR_Exc) Then
            ...
            With GelUMC(CallerID)
                ...
                With .UMCs(lngUMCIndexLight)
                    strPairInfo = strPairInfo & lngUMCIndexLight & strSepChar
                    strPairInfo = strPairInfo & .ClassMW & strSepChar
                    strPairInfo = strPairInfo & .ClassAbundance & strSepChar
                    strPairInfo = strPairInfo & .MinScan & strSepChar
                    strPairInfo = strPairInfo & .MaxScan & strSepChar
                    strPairInfo = strPairInfo & .ClassCount & strSepChar
                        
                    lngAvgScanNumber = (.MinScan + .MaxScan) / 2
                    dblAvgGANET = ScanToGANET(CallerID, lngAvgScanNumber)
                ...
                With .UMCs(lngUMCIndexHeavy)
                    strPairInfo = strPairInfo & lngUMCIndexHeavy & strSepChar
                    strPairInfo = strPairInfo & .ClassMW & strSepChar
                    strPairInfo = strPairInfo & .ClassAbundance & strSepChar
                    strPairInfo = strPairInfo & GelP_D_L(CallerID).Pairs(lngPairInd).P2DltCnt & strSepChar
                    strPairInfo = strPairInfo & .MinScan & strSepChar
                    strPairInfo = strPairInfo & .MaxScan & strSepChar
                    strPairInfo = strPairInfo & .ClassCount & strSepChar
                End With
            ...
            Else                                    'identified
               For lngMatchIndex = PIDInd1(lngPairInd) To PIDInd2(lngPairInd)
                    ...
                    dblGANETError = dblAvgGANET - mMTNET(lngMassTagIndexPointer)
...
Private Function CheckNAtomsVsDeltaCount(MW As Double, DltCnt As Long, AMTIndex As Long) As Boolean
    ...
    'allow for +/-1(2) error for masses over 2000 (4000) Da
    blnValidHit = False
    If MW > 2 * glN14_N15CorrMW Then
...
''Private Function ExportIDPairsToPeakResultsTable(Optional ByRef lngMDID As Long, Optional ByVal blnUpdateGANETForAnalysisInDB As Boolean, Optional ByRef lngErrorNumber As Long, Optional ByVal blnExportAllIonsInUMC As Boolean = False, Optional ByVal blnExportExcludedPairs As Boolean = True) As String
'''---------------------------------------------------
...
'''However, if blnExportAllIonsInUMC = True, then we will, instead, export the mass of all of the ions (peaks) in each UMC
'''
...
''    ExportIDPairsToPeakResultsTable = "Error: Unable to establish a connection to the database"
''    Exit Function
''End If
''
...
''Set prmResType = cmdPutNewPeak.CreateParameter("Type", adInteger, adParamInput, , FPR_Type_Standard)    ' This will get updated below
...
''
''Me.Caption = "Exporting pairs to DB: 0 / " & Trim(PCount)
''
...
''For i = 0 To PCount - 1
''    If i Mod 25 = 0 Then
''        Me.Caption = "Exporting pairs to DB: " & Trim(i) & " / " & Trim(PCount)
''        DoEvents
''        TraceLog 3, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Examining lngPairInd = " & Trim(i)
''    Else
''        TraceLog 2, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Examining lngPairInd = " & Trim(i)
''    End If
''
''    If PIDCnt(i) > 0 And _
''       (blnExportExcludedPairs Or GelP_D_L(CallerID).Pairs(i).STATE <> glPAIR_Exc) Then        'this pair is identified
''
''       TraceLog 3, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Exporting pair " & Trim(i)
''
...
''            prmResType.value = lngPeakFPRTypeLight
...
''            prmUMCBestFit.value = Round(UMCStat2(IndL, 6), FIT_PRECISION)
''            prmUMCAvgMW.value = Round(.ClassMW, MASS_PRECISION)              ' This is usually the median mass of the class, not the average mass
''
''            Debug.Assert UMCStat2(IndL, 2) = .MinScan
''            Debug.Assert UMCStat2(IndL, 3) = .MaxScan
''            Debug.Assert UMCStat2(IndL, 8) = .ClassCount
''            '' Debug.Assert UMCStat2(IndL, 4) = .ClassAbundance                ' This will not be true if the UMC has scan holes and the abundance for the holes was interpolated
''            Debug.Assert Round(UMCStat2(IndL, 1), 0) = Round(.ClassMW, 0)
''
''            For j = 0 To .ClassCount - 1
''               ' Note: We only export all of the members of each UMC if blnExportAllIonsInUMC = True
''               If .ClassMInd(j) = .ClassRepInd Or blnExportAllIonsInUMC Then
''                    nInd = .ClassMInd(j)
...
''                        lngMassTagIndexOriginal = mMTOrInd(mMTInd(mUMCMatchStats(k).IDIndex))
''                        prmMassTagID = AMTData(lngMassTagIndexOriginal).ID     'MT tag ID
...
''            prmResType.value = lngPeakFPRTypeHeavy
''            prmUMCFirstScan.value = .MinScan
''            prmUMCLastScan.value = .MaxScan
''            prmUMCCount.value = .ClassCount
''            prmUMCAbundance.value = .ClassAbundance
''            prmUMCBestFit.value = Round(UMCStat2(IndH, 6), FIT_PRECISION)
''            prmUMCAvgMW.value = Round(.ClassMW, MASS_PRECISION)              ' This is usually the median mass of the class, not the average mass
''
''           For j = 0 To .ClassCount - 1
''                ' Note: We only export all of the members of each UMC if blnExportAllIonsInUMC = True
...
''Next i
''
''TraceLog 4, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Exported data: pairs Match Count = " & ExpCnt
''
''' MonroeMod
...
''strExportStatus = ExpCnt & " associations between MT tags and FTICR peaks exported."
''Set cmdPutNewPeak.ActiveConnection = Nothing
''cnNew.Close
''
''TraceLog 5, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", strExportStatus
''
''If blnUpdateGANETForAnalysisInDB Then
''    ' Export the the GANET Slope, Intercept, and Fit to the database
''    TraceLog 5, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Call ExportGANETtoMTDB"
''    With GelAnalysis(CallerID)
''        strExportStatus = strExportStatus & vbCrLf & ExportGANETtoMTDB(CallerID, .GANET_Slope, .GANET_Intercept, .GANET_Fit)
''    End With
''End If
''
''ExportIDPairsToPeakResultsTable = strExportStatus
''lngErrorNumber = 0
''Exit Function
''
''err_ExportMTDB:
''ExportIDPairsToPeakResultsTable = "Error: " & Err.Number & vbCrLf & Err.Description
''lngErrorNumber = Err.Number
''TraceLog 5, "frmSearchMTPairs->ExportIDPairsToPeakResultsTable", "Error occurred: " & Err.Description
''
''On Error Resume Next
